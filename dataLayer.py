...
Apply incremental map-reduce mechanism to update collection 1 to collection 2
Incremental apply time stamp for storing the update

Collection 1 has unique compound key on "text" and "documentId",
which means if the "text" is the same
but with different "documentId", it still allow to store both information.
When doing add, entry will be directly
added in to the collection using the format shown as following
if the compound key does not exist, otherwise,
update will applied, and also renew the timestamp.
This allow atomic operation on individual entry when
doing add, update, remove and etc.

Collection 1 Example:
content1 and content2 can exist in collection 1 at the
same time, after done with the 'put' operation,
map reduce would be applied to aggregate the newly inserted/updated
entry into the collection 2.
...
Collection 2 is handled by map reduce,
incrementally map the newly added or updated entry from collection 1,
and do reduce to the mapped entry from collection 1
and the already existed entry from collection 2.
Map reduce provided by mongodb should be considered as thread safe.

Collection 2 Example:
The following example shown outcome
of applying map reduce on content1 and content2
...
class DataLayer:
    """
    constructor for DataLayer, provide a way for user declare the database and
    ...
    def __init__(
        ...
        try:
            ...
            #  create the unique compound index on text
            #  and documentId in decreasing order
    ...
    def put(self, contents):
        ...
        then applied map reduce on the newly updated entry,
        then stores the result to collection2.
        ...
        Every content has "text", "documentId", "document" as field,
        where "document" map to an object with "tf", "occurrences" as field.
        :return: True if all insert success, if one fail,
        raise DataBasePutFail Exception
        """
        try:
    ...
    def map_reduce_aggregation(self, last_update):
        """
        map_reduce_aggregation will be called by
        the put when every update happen.
        map the newly update entry in collection1 and reduce
        with existing entry in collection2
        the example outcome in collection2 shown at the top
        :param last_update: specified timestamp to define the
        newly updated content. map reduce will
        only be applied on the content with timestamp larger last_update.
        :return: the result of map reduce, if fail,
        throw DataBaseAggregationFail which extends DataBasePutFail Exception
        """
        try:
            #  define the mapper,
    ...
    def delete_text(self, document_id, texts):
        ...
        :param texts: a list of text need to delete the entry
        of specified documentId
        ...
        try:
            ...
            for text in texts:
                ...
                self.collection.delete_one(
                    {"text": text, "documentId": document_id}
                )
        except Exception:
    ...
    def get(self, texts):
        ...
        :param texts: is a list of text, for example
        ["my_word", "lambda", "test"].
        :return: a list of result, if text does not exist,
        None value will be included
        """
        try:
    ...
    def debug_print_collection(self):
        ...
        """
        print(
            "document found in collection: {}".format(
    ...
    def debug_print_mr_collection(self):
        ...
        """
        print("document found in collection: {}".format(
            self.mr_collection.count_documents({})
            )
        )
